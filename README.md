...
"Полезное"):** в отличие от срочных новостей и платного продвижения (пункты
выше — обязательные, идут всем подписчикам района без возможности
отключить), эти категории по умолчанию ВЫКЛЮЧЕНЫ — житель включает их сам в
"Настройках" приложения (`NotificationPreferences`), каждая — отдельный FCM
topic (`district_<id>_events`, `district_<id>_vacancies`,
`district_<id>_free_announcements`; `useful_offers` — общий на всё
приложение, без района).
