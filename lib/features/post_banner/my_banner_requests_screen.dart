...
class MyBannerRequestsScreen extends StatefulWidget {
  const MyBannerRequestsScreen({super.key});

  @override
  State<MyBannerRequestsScreen> createState() => _MyBannerRequestsScreenState();
}

class _MyBannerRequestsScreenState extends State<MyBannerRequestsScreen> {
  ...
  Future<void> _load() async {
    final requests = await LocalStorageService().getMyBannerRequests();
    if (!mounted) return;
    setState(() {
      _requests = requests;
      _isLoading = false;
          ...
          : _requests.isEmpty
              ? Center(
                  child: Padding(
                    ...
                    child: Text(
                      'Вы ещё не отправляли заявки на баннер с этого устройства',
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Размещение на $durationDays дней',
