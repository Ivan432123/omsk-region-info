...
class MyVacancyRequestsScreen extends StatefulWidget {
  const MyVacancyRequestsScreen({super.key});

  @override
  State<MyVacancyRequestsScreen> createState() =>
      _MyVacancyRequestsScreenState();
}

class _MyVacancyRequestsScreenState extends State<MyVacancyRequestsScreen> {
  ...
  Future<void> _load() async {
    final requests = await LocalStorageService().getMyVacancyRequests();
    if (!mounted) return;
    setState(() {
      _requests = requests;
      _isLoading = false;
          ...
          : _requests.isEmpty
              ? Center(
                  child: Padding(
                    ...
                    child: Text(
                      'Вы ещё не отправляли заявки на вакансию с этого устройства',
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                const Text(
                    'Вакансия будет опубликована на 30 дней после одобрения',
